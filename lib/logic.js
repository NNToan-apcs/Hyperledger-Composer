...
/**
 * Track the Transfer of a commodity from one trader to another
 * @param { org.trading.net.BuyCommodity} Buy - the Transfer to be processed
 * @transaction
 */

async function buyCommodity(Buy) {
    ...
    if (currentParticipant.getFullyQualifiedIdentifier() === Buy.commodity.owner.getFullyQualifiedIdentifier()) {
        throw new Error('Owner should not buy their assets');
    }
    
    // Update new owner for community
    Buy.commodity.owner = currentParticipant;

    let assetRegistry = await getAssetRegistry('org.trading.net.Commodity');

    // emit a notification that a Buy has occurred
    let SuccessNotification = getFactory().newEvent('org.trading.net', 'BuyCommoditySuccessNotification');
    SuccessNotification.commodity = Buy.commodity;
    SuccessNotification.newOwner = Buy.newOwner;
    emit(SuccessNotification);

    // persist the state of the course
    await assetRegistry.update(Buy.commodity);
