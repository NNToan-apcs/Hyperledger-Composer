...
async function transferCommodity(Transfer) {
    ...
    if (!getParticipantRegistry('org.trading.net.Trader').exists(newOwner.getIdentifer()))
    {
...
/**
 * Track the Purchase of a commodity from one trader 
 * @param { org.trading.net.BuyCommodity} Purchase - the Purchase to be processed
 * @transaction
 */

async function buyCommodity(Purchase) {
    ...
    console.log("BBBBBBBBBBBBBBBBBBBBBBBBb");
    if (currentParticipant.getFullyQualifiedIdentifier() === Purchase.commodity.owner.getFullyQualifiedIdentifier()) {
        throw new Error('Transaction can only be submitted not by the owner of the commodity');
    }
    console.log("CCCCCCCCCCCCCCCCCCCCCCCCCCC");
    
    if (!getAssetRegistry('org.trading.net.Trader').exists(Purchase.commodity.getIdentifer()))
    {
        throw new Error('commodity not exists');
    }
    console.log("DDDDDDDDDDDDDDDDDDDDDDDDDDD");
    // Update new owner for community
    Purchase.commodity.owner = currentParticipant;
    let assetRegistry = await getAssetRegistry('org.trading.net.Commodity');

    // emit a notification that a Purchase has occurred
    let SuccessNotification = getFactory().newEvent('org.trading.net', 'BuyCommoditySuccessNotification');
    SuccessNotification.commodity = Purchase.commodity;
    SuccessNotification.newOwner = currentParticipant;
    emit(SuccessNotification);

    // persist the state of the course
    await assetRegistry.update(Purchase.commodity);
