...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be toan-network@0.0.5',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('toan-network@0.0.5.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('Mobile component should be loadable',() => {
      page.navigateTo('/Mobile');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Mobile');
      });
    });

    it('Mobile table should have 5 columns',() => {
      page.navigateTo('/Mobile');
